...
type Row = {
  ...
  plan: { reaction_min: number | null; resolution_min: number | null; total_min: number };
  fakt: { reaction_min: number | null; resolution_min: number | null; total_min: number };
...
export function SlaByRequestType() {
  ...
  return (
    <div className="p-6 flex flex-col gap-3" style={{ height: "100%" }}>
      ...
      <div style={{ display: "flex", gap: 16, fontSize: "0.68rem", color: "var(--soft)" }}>
        ...
        <span style={{ display: "flex", alignItems: "center", gap: 5 }}><i style={{ width: 10, height: 10, borderRadius: 3, background: FAKT, display: "inline-block" }} /> {t("sla_fakt")}</span>
          ...
          <tbody>
            {filtered.map((r) => (
              <tr key={r.name} onClick={() => setSel(r)}
                ...
                <td style={{ padding: "7px 8px", textAlign: "center" }}><PvF plan={r.plan.reaction_min} fakt={r.fakt.reaction_min} /></td>
                <td style={{ padding: "7px 8px", textAlign: "center" }}><PvF plan={r.plan.resolution_min} fakt={r.fakt.resolution_min} /></td>
                <td style={{ padding: "7px 8px", textAlign: "center" }}><PvF plan={r.plan.total_min} fakt={r.fakt.total_min} /></td>
      ...
      <AnimatePresence>
        {sel && <DetailModal row={sel} onClose={() => setSel(null)} />}
      </AnimatePresence>
    </div>
  );
}
...
function DetailModal({ row, onClose }: { row: Row; onClose: () => void }) {
  ...
  const chartData = [
    { k: t("sla_react"), Plan: row.plan.reaction_min ?? 0, Fakt: row.fakt.reaction_min ?? 0 },
    { k: t("sla_resol"), Plan: row.plan.resolution_min ?? 0, Fakt: row.fakt.resolution_min ?? 0 },
    { k: t("sla_total_word"), Plan: row.plan.total_min ?? 0, Fakt: row.fakt.total_min ?? 0 },
  ];
      ...
      <motion.div
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr 1fr 1fr", gap: 1, background: "var(--divider)", borderRadius: 10, overflow: "hidden" }}>
          {[
            ...
            [t("sla_react"), fmtMin(row.plan.reaction_min), fmtMin(row.fakt.reaction_min), delta(row.plan.reaction_min, row.fakt.reaction_min)],
            [t("sla_resol"), fmtMin(row.plan.resolution_min), fmtMin(row.fakt.resolution_min), delta(row.plan.resolution_min, row.fakt.resolution_min)],
            [t("sla_total_word"), fmtMin(row.plan.total_min), fmtMin(row.fakt.total_min), delta(row.plan.total_min, row.fakt.total_min)],
          ].map((cells, ri) => (
            cells.map((c, ci) => (
