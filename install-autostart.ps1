...
# 2) Creates a Desktop icon "SLANEST" + a Startup-folder shortcut.
...
try {
  ...
  Register-ScheduledTask -TaskName "SLANEST" -Action $action -Trigger $trigger `
    -Settings $settings -Description "SLANEST resilient web app (port 8090)" -Force | Out-Null
...
try {
  if (-not (Get-NetFirewallRule -DisplayName "SLANEST 8090" -ErrorAction SilentlyContinue)) {
    New-NetFirewallRule -DisplayName "SLANEST 8090" -Direction Inbound -Action Allow `
      -Protocol TCP -LocalPort 8090 -Profile Any | Out-Null
...
# Desktop "SLANEST" — opens the app (ensures server up first)
...
Write-Host "`nDone. App will be at http://localhost:8090 (and on your LAN)." -ForegroundColor Cyan
